...
1. As a Pasien, I want to view a landing page with clear value proposition, counselor highlights, pricing, client testimonials, gallery, and FAQ, so that I understand what Solulu offers and feel confident to proceed.
...
4. As a Pasien, I want to complete an informative SRQ-20 screening questionnaire (20 yes/no questions) before booking, so that the counselor has clinical context about my emotional state.
5. As a Pasien, I want my screening score and summary to be informative without complex blocking or clinical waiver modals, keeping the booking process frictionless and non-intimidating.
6. As a Pasien, I want prominent supportive crisis hotline information (Kemenkes 119 Ext 8, SEJIWA) accessible whenever I need immediate emergency help.
...
10. As a Pasien, I want to apply a voucher code during checkout (where permitted by the counselor type's voucher policy) and see the discounted price, so that I can save money.
11. As a Pasien, I want my selected slot to be held (reserved) for 15 minutes while I complete payment, so that no one else takes it.
12. As a Pasien, I want flexible payment options: automated payment via Xendit (QRIS, e-Wallet, Virtual Account) or Manual Bank Transfer with admin verification, so that I can pay with my preferred method.
13. As a Pasien, I want to see a success page with a prominent "Buka Halaman Sesi Saya Sekarang" button after payment/confirmation, so that I can immediately access my session page.
...
32. As a Mitra Konselor, I want to see a list of my upcoming sessions (today + 7 days) with patient name, initial notes, and informative SRQ-20 score, so that I can prepare clinically.
...
40. As an Admin, I want an operational dashboard showing live session badges, today's sessions, pending manual payments queue, monthly gross revenue, and pending counselor applications, so that I have complete operational awareness.
41. As an Admin, I want to view sessions in both Table View and Calendar View (weekly and monthly), with a side drawer detailing patient contact, WhatsApp direct link, payment status, and Zoom URLs.
42. As an Admin, I want to verify manual bank payments (BCA, Mandiri, BRI, QRIS Statis) via `ManualPaymentModal`, enter transaction reference/mutation ID, check Zoom concurrency availability, and immediately issue Zoom room + confirmation email to patient.
43. As an Admin, I want to manage the list of verified counselors, view their profiles, and deactivate them if needed, so that I maintain platform quality.
44. As an Admin, I want to review counselor applications, preview documents in-modal (KTP, CV, Ijazah, STR), and approve or reject them with automated Supabase invite email.
...
46. As an Admin, I want to configure 2 Zoom accounts with encrypted client secrets (AES-256-GCM), a 90-minute visual concurrency timeline (ADR-0002), and an interactive Safety Lock audit listing the exact sessions locking each account.
47. As an Admin, I want to set base price, promotional sale toggle (`isSaleActive`), sale price, and voucher permission (`allowVoucher`) per counselor type, so that pricing and promotions are finely tuned.
48. As an Admin, I want to create and manage voucher codes with discount type (fixed/percentage), value, quota, expiry, and active status, so that I can run targeted marketing campaigns.
49. As an Admin, I want to curate client testimonials at `/admin/testimonials` (anonymous name, star rating, quotes, topic tag, active toggle, and featured on home) to display real reviews on the landing page.
...
52. As an Admin, I want reschedule to be dynamically blocked if the session is less than 12 hours away (H-12 rule), with clear explanatory feedback and countdown.
53. As an Admin, I want to manage my profile and emergency support settings at `/admin/profile` (WhatsApp help desk, email, notification preferences, and password change).
54. As an Admin, I want a Command Palette (`⌘K`) accessible anywhere in the admin shell for rapid keyboard-driven navigation across sessions, counselors, settings, and pages.

### System / Background

55. As the system, I want the Xendit webhook handler to validate the callback token, check idempotency, update transaction/booking/schedule status, dispatch a QStash message, and return HTTP 200 in under 1 second, so that Vercel's 10-second timeout is never hit.
56. As the system, I want the QStash fulfill-booking worker to allocate a Zoom account dynamically (pick whichever of the 2 accounts has no overlapping confirmed session), create the Zoom meeting, save join/start URLs, and send confirmation + session link emails to patient and counselor, so that fulfillment is decoupled and reliable.
57. As the system, I want Zoom OAuth tokens cached in the database with 3500-second TTL to prevent rate limiting (HTTP 429), so that token requests are minimized.
58. As the system, I want a QStash cron every 5 minutes calling `/api/cron/cleanup-slots` (protected by CRON_SECRET bearer token) to atomically: (1) release expired reserved slots back to available, (2) cancel associated bookings, (3) expire associated transactions, (4) rollback voucher `usedCount`, (5) auto-archive sessions >2 hours past `endTime` as completed, so that stale state is cleaned up.
59. As the system, I want the slot hold `reserved_until` set to NOW() + 17 minutes (2-minute buffer over Xendit's 15-minute invoice expiry), so that webhook latency doesn't cause premature cleanup.
60. As the system, I want middleware to check Supabase Auth session cookies and enforce RBAC: `/admin/*` requires role=admin, `/counselor/*` requires role=counselor, with appropriate redirects, so that protected routes are secure.
61. As the system, I want `/session/[token]` to be publicly accessible without login, validated against the database access_token, so that guest patients can access their sessions.
62. As the system, I want rate limiting on `/cek-sesi` (3 req/15 min/IP) via Upstash Redis + `@upstash/ratelimit`, so that enumeration attacks are prevented.
63. As the system, I want all R2 images in Next.js to use `unoptimized={true}`, so that Vercel's 1000/month image optimization quota is preserved.
64. As the system, I want all file uploads (documents, gallery, avatar) to go directly to Cloudflare R2 via presigned PUT URLs (bypassing Vercel's 4.5MB limit), so that large files work reliably.
...
- **Schema additions from Prototype**:
  - Add `testimonials` table: `id`, `clientName`, `isAnonymous`, `anonymousDisplay`, `sessionCode`, `counselorName`, `counselorType`, `rating`, `quoteHighlight`, `comment`, `topic`, `isActive`, `isFeatured`, `createdAt`, `updatedAt`.
  - In `transactions`: `xenditInvoiceId` & `xenditPaymentUrl` are nullable to support Manual Bank Transfer; add `paymentProvider` ('xendit' | 'manual'), `referenceNumber` (text), and `adminNotes` (text).
  - In `platformPricing`: add `isSaleActive` (boolean, default false) and `allowVoucher` (boolean, default true).
  - In `screenings`: simplified informative schema (`answers` JSONB, `totalScore` integer, `hasSuicidalThoughts` boolean, `recommendedType` counselorTypeEnum).
  - In `bookings`: `screeningId` is made nullable/informative; `bypassedRecommendation` & `waiverAcceptedAt` are deprecated/optional.
  - `ktpR2Key` (text, not null) and `diplomaR2Key` (text, not null) in `counselorApplications`.
  - `voucherId` (uuid, nullable) on `transactions` for audit and rollback.
  - `schedules.endTime` is auto-computed (`startTime + 90 minutes`).
  - `schedules.reserved_until` uses NOW() + 17 minutes.
- **Session duration**: Fixed 90 minutes.

### Payment & Fulfillment Dual-Path

- **Path A: Automated Xendit Gateway**: Webhook `/api/webhooks/xendit` -> QStash `/api/jobs/fulfill-booking`.
- **Path B: Manual Bank Transfer Verification**: Admin confirms payment in `ManualPaymentModal` -> Server action executes dynamic Zoom host allocation, updates transaction & schedule status, and sends Resend confirmation email immediately.

### Informative SRQ-20 Screening

- 20 yes/no questions answered during booking flow.
- Purpose is purely clinical context for the counselor (displayed on session drawer and upcoming sessions list).
- No blocking waiver modal, no forced counselor restriction.

### UI & Design System

- Built per [DESIGN.md](file:///home/muhalfatih/solulu-web/DESIGN.md): Creative North Star "The Grounding Sanctuary", Obsidian Ink & Healing Emerald palette, Geist Sans typography with WCAG AA compliance.
- Interactive Command Palette (`⌘K`) for rapid operational jumping.
- Dynamic H-12 Reschedule enforcement.
- 90-minute visual concurrency timeline in Zoom management.
...
- `/admin/testimonials`: client review curation module.
- `/admin/profile`: admin security and contact settings.
...
- Landing page: hero, value prop, cara kerja, konselor featured, pricing, client testimonials, gallery, FAQ, footer.
- Mobile-first, WCAG AA contrast, calming/grounding visual tone per `DESIGN.md`.
- `lang="id"` on html element.

### Reschedule

- Admin-only, semi-automatic: dropdown to pick new slot + automatic email to patient.
- Blocked dynamically if session is <12 hours away (H-12 rule).

## Testing Decisions

### Testing Seams

- **Primary seam: Server Actions.** All business logic is encapsulated in server actions. Tests call actions directly against a test database (Supabase local via Docker or test project). This is the highest testable boundary and covers: concurrency guard, voucher validation (including `allowVoucher` rule), safety lock, slot hold, manual payment confirmation, SRQ-20 scoring, RBAC enforcement.
...
- Edge cases to cover: concurrent slot hold (race condition), voucher at quota limit or role restriction, safety lock with future sessions, overlap guard with 2 sessions already active, manual payment confirmation Zoom assignment.
